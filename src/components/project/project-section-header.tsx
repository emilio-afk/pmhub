...
function SectionStatPill({ item }: { key?: React.Key; item: SectionStat }) {
  ...
  return (
    <Badge variant="outline" className="gap-3 px-3 py-2 text-left" aria-label={`${item.label}: ${item.value}`}>
      ...
      <span className="ui-kicker text-[12px] font-medium">{item.label}</span>
    </Badge>
  );
}
      ...
      <div className="flex flex-col gap-5 xl:flex-row xl:flex-nowrap xl:items-center xl:justify-between">
        <div className="min-w-0 xl:flex-none">
          <div className="flex items-center gap-3">
            {Icon && (
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-slate-200/80 bg-slate-50/90 text-slate-600">
                <Icon size={17} />
              </div>
            )}
            <div className="min-w-0">
              <h2 className="ui-text-main text-[28px] font-bold tracking-tight">{label}</h2>
            </div>
          </div>
        </div>
        {(action || hasStats) && (
