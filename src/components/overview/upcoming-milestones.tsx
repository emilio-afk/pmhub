  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      {milestones.map(milestone => (
        <div key={milestone.id} className="ui-panel-soft rounded-2xl p-5 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="ui-text-main text-[15px] font-semibold tracking-tight">{milestone.title}</p>
              <p className="ui-text-subtle mt-1.5 text-sm">{dueDateLabelById[milestone.id]}</p>
            </div>
            <span
          ...
          <Progress
            ...
            className="mt-4 h-2 rounded-full bg-white/90 [&_[data-slot=progress-indicator]]:bg-[#2b6f8f]"
          />
        </div>
      ))}
    </div>
